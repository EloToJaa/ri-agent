...
Automatic compaction runs before each model request when estimated conversation history reaches 32,000 tokens. Set `--auto-compact-tokens N` to change the threshold, or `--auto-compact-tokens 0` to disable it. It uses a local excerpt summary, only keeps changes that reduce estimated size, and never summarizes the active turn. Estimates use serialized history bytes divided by four, excluding tool definitions; they are not model-specific context limits. A large active turn or retained recent history can still exceed the threshold. Compaction progress appears in CLI, TUI, and JSON events.
