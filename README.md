...
- **Esc**: cancel the active turn when no picker or completion is open. Pickers and completions consume Esc first. Cancellation stops model requests and cancels an actively awaited Bash command; filesystem tools and Lua hooks finish before the turn stops, and remaining tools are skipped. A blocked Lua callback can delay cancellation. Applied changes are retained.
...
Provider requests retry up to two times after typed connection/timeout failures or HTTP 408, 429, 500, 502, 503, and 504. Backoff starts at 500 ms, doubles, and adds up to 250 ms jitter. `Retry-After` supports seconds and HTTP dates; delays over one minute return an error instead of retrying early. Error text is never searched to decide retries. Authentication, malformed-response, model-access, and other non-transient errors fail immediately. A cancelled request interrupts both the request and its backoff; no tool call is executed until a complete provider response is received. Retried partial text is marked incomplete.

JSON `model_request` events report usage when supplied, termination reason, and request latency. CLI/TUI display available token usage. Session metrics persist request counts, input/output tokens, reported cost, elapsed request time, and the number of requests without usage. Reported costs and token counts are partial when a provider omits metadata; they are not billing totals. Length-limited responses pause the session and never execute their tool calls; filtered or otherwise incomplete responses fail safely. OpenRouter sends the response reserve as `max_tokens`; Codex's backend controls its output limit.
...
Bash accepts `cwd`, `timeout_ms` (up to 12 hours), and `yield_time_ms` (0–60,000). Without a yield duration it waits for completion. To run a build or server asynchronously, use `{"command":"cargo test","yield_time_ms":1000}`; a running result includes `command_id`. Then use `{"action":"poll","command_id":"…","yield_time_ms":1000}` for new output, or `{"action":"cancel","command_id":"…"}` to stop it. Stdout/stderr stream to the UI and JSON `command_output` events, bounded by the configured output limit. Polling consumes buffered output. Results distinguish running, exit status, cancellation, and timeout.

Handles belong to the current session/process and cannot be resumed after restarting. Dropping or clearing a session cancels its commands. On Unix, cancellation, timeout, and shell exit terminate the command's process group, including ordinary descendants; deliberately detached processes can escape that group. On other platforms only the direct child is terminated. No stdin or PTY support is provided. At most 64 command records are retained before completed records are evicted.
